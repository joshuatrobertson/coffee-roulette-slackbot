...
def generate_weekly_message(date):
    ...
    for (month, day), season_name in seasons.items():
        ...
        if today == season_start or is_first_monday(today, season_start):
            event = (f"Your role is to help users create engaging and friendly Slack posts for organizing coffee "
                     ...
                     f"users to"
                     f"engage by reacting. For the 3 emojis make sure they are different After presenting the "
    ...
    prompt = (f"Your role is to help users create engaging and friendly Slack posts for organizing coffee "
                     ...
                     f"welcoming community spirit while keeping the message clear and organized. The post should "
                     f"include a question"
                     f"relevant to the day it falls on, in this case {today_str} Each post includes one "
                     f"question related to {today_str} with three"
                     f"emoji-reactable answers. Emojis are placed before each short, single-line answer, inviting "
                     f"users to"
                     f"engage by reacting. For the 3 emojis make sure they are different After presenting the "
                     f"question and answers,"
